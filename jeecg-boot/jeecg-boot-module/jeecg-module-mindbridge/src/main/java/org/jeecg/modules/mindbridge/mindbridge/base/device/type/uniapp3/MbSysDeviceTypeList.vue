...
<template>
  <PageLayout navTitle="设备类型"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './MbSysDeviceTypeData';
defineOptions({
  name: 'MbSysDeviceTypeList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/mindbridge/base/device/type/mbSysDeviceType/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/mindbridge/base/device/type/mbSysDeviceType/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
